...
func (kernel *Kernel) FuzzerInstance(ctx context.Context, inst *vm.Instance, updInfo dispatcher.UpdateInfo) {
	...
	if len(reps) > 0 {
		// Log all detected crashes
		select {
		case <-ctx.Done():
		default:
			for i, crashRep := range reps {
				if i == 0 {
					log.Logf(0, "kernel %s: VM crash detected: %s", kernel.cfg.Name, crashRep.Title)
				} else {
					log.Logf(0, "kernel %s: cascade crash #%d: %s", kernel.cfg.Name, i, crashRep.Title)
				}
			}
		}
	}
...
func (kernel *Kernel) runInstance(ctx context.Context, inst *vm.Instance,
	...
	defer cancel()
	_, reports, err := inst.Run(ctxTimeout, kernel.reporter, cmd,
